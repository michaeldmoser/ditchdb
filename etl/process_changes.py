...
from ditchdb.models import OrionProperty, Property, OrionOwner, OrionParty, Owner, Party, Changes, Partyaddr, OrionPartyaddr  # noqa E402

# Create all new parties
new_parties = OrionParty.objects \
        .filter(properties__indistrict=True) \
    ...
    with transaction.atomic():
        ...
        for owner in owners:
            ...
            Changes.objects.create(
                property=property,
                change_type=Changes.NEW_PROPERTY 
            )

# Track removed properties
parcels = Property.objects.exclude(orion_property__in=OrionProperty.objects.in_district())
for parcel in parcels:
    Changes.objects.create(
        property=parcel,
        change_type=Changes.PROPERTY_REMOVED
    )
    

# Track changes in property details
parcels = (Property.objects 
        .exclude(addr_number=F('orion_property__addr_number')) 
...
for parcel in parcels:
    Changes.objects.create(
        property=parcel,
        change_type=Changes.PROPERTY_CHANGED
    ) 


owners = Owner.objects.all()
for owner in owners:
    if not OrionOwner.objects.filter(propertyid=owner.property_id).filter(party_id=owner.party_id).exists():
        Chagnes.objects.create(
                property=owner.property,
                change_type=Changes.OWNERSHIP_CHANGED
                )

addresses = Partyaddr.objects.filter(defaultaddress=True)
for address in addresses:
    if not (OrionPartyaddr.objects
            .filter(defaultaddress=True)
        ...
        for owner in owners:
            Changes.objects.create(
                    property=owner.property,
                    change_type=Changes.ADDRESS_CHANGED
                    )
